...
## Exporting Task Notes

You can export a task together with its notes from the task context menu. The export is a Markdown file when the notes only contain text and ordinary links. If the notes contain local screenshots or pasted clipboard images, the export is a ZIP archive containing the Markdown file and an `images/` folder, with the Markdown image links rewritten to point to the packaged image files.
